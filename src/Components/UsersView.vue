...
<template>
  <div class="text-black md:w-[60vw] w-[80vw] flex flex-col items-center" v-if="usersList.length">
    <input
      type="search"
    ...
    <p v-if="filteredUsers.length === 0" class="text-center w-full">Neither username nor type contains {{ `"${searchInput}"` }}</p>
  </div>
